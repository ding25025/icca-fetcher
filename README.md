...
## 最後一次成功寫入是什麼時候（state.js）

撈取的時間窗是相對的（「現在往回 N 分鐘」），跑完就忘：服務停一小時再起來，
只會補最近幾分鐘，中間那段沒有任何地方記得。所以每一輪成功寫入之後，
時間會記在工作目錄下的 **`.sink-state.json`**：

```json
{
  "vitals": {
    "startedAt": "2026-08-18 09:12:03",
    "startedAtMs": 1787015523000,
    "finishedAt": "2026-08-18 09:12:41",
    "rows": 38, "written": 214,
    "table": "[dbo].[CDSUnvalidatedData]",
    "lastError": null, "lastErrorAt": null
  }
}
```

`startedAt` 是**那一輪開始撈的時間**（不是寫完的時間）——要補就從這個點往後撈，
才不會漏掉「撈完到寫完」之間新進來的資料；重疊的部分寫入時本來就會被擋掉。
失敗只更新 `lastError`，不動 `startedAt`：水位線只有成功才前進，缺口才看得出來。

`server.js` 常駐時，這份摘要會出現在 `/health` 的 `lastWrite`，
裡面直接給好 `suggestWindowMinutes`（要補的話該下多大的窗）；
啟動時也會印一行在 log 上。命令列 `--to-db` 跑完一樣會記
（但 `vitals.js --site` 只跑部分站台，那不算完整的一輪，不記）。

**最多補 24 小時**：生命徵象的來源是環狀表，26 張、一張約一小時，
再往前就被覆蓋；神經評估雖然沒有被覆蓋的問題，也照同一個上限，
一次開太大的窗會壓到正式機。缺口超過上限時 `/health` 會標 `gapBeyondBackfill`。
上限用環境變數 `ICCA_MAX_BACKFILL_HOURS` 調，狀態檔路徑用 `ICCA_STATE` 換。

這個檔**只記錄，不自動補**：排程行為完全沒變，開多大的窗還是人決定。
刪掉它只會失去「該從幾點補」這個起點，不影響撈取。
